...
Private Sub cmdCreateLink_Click()
    ...
    '
    fCreateShellLink txtLinkName.Text, txtExeName.Text, txtWorkDir.Text, txtCmdArgs.Text, txtIconFile.Text, CLng(txtIconIndex.Text), CLng(txtShowCmd.Text)
End Sub

Private Sub cmdGetLinkInfo_Click()
    Dim sLnkFile  As String  ' Link file name
    Dim sExeFile  As String  ' Link - Exe file name
    Dim sWorkDir  As String  '      - Working directory
    Dim sExeArgs  As String  '      - Command line arguments
    Dim sIconFile As String  '      - Icon File name
    Dim lIconIdx  As Long    '      - Icon Index
    Dim lShowCmd  As Long    '      - Program start state.
    '
    ' Get link file name and clear output variables.
    '
    sLnkFile = txtLinkName.Text
    txtExeName.Text = ""
    txtWorkDir.Text = ""
    txtCmdArgs.Text = ""
    txtIconFile.Text = ""
    ...
    txtShowCmd.Text = ""
    '
    ' Get Info for a shortcut file.
    '
    fGetShellLinkInfo sLnkFile, sExeFile, sWorkDir, sExeArgs, sIconFile, lIconIdx, lShowCmd
    '
    ' Display the output.
    '
    txtLinkName.Text = sLnkFile
    txtExeName.Text = sExeFile
    txtWorkDir.Text = sWorkDir
    txtCmdArgs.Text = sExeArgs
    txtIconFile.Text = sIconFile
    txtIconIndex.Text = Val(lIconIdx)
    txtShowCmd.Text = Val(lShowCmd)
End Sub
...
Private Sub cmdGetPath_Click()
    ...
    Dim lId   As Long
    '
    ' Get system folder ID from the combo box.
    '
    lId = cmbSysFolders.ItemData(cmbSysFolders.ListIndex)
    '
    ' Get system folder path from the id and display it.
    '
    If fGetSystemFolderPath(Me.hwnd, lId, sPath) Then Call pSetDefaults(sPath)
End Sub
...
Private Sub Form_Load()
    '
    ' Display this application's path.
    '
    Call pSetDefaults(App.Path & "\")
    '
    ' Add ID's for system folders to combo box.
    '
    With cmbSysFolders
        .AddItem "DESKTOP"
        .ItemData(.NewIndex) = 0
...
Private Sub pSetDefaults(spth As String)
    ...
    txtLinkName.Text = spth & "testlink.lnk"                ' Create a full path name for link file
    ...
    txtProgramGroup.Text = spth & "Test Link Program Group" ' Set default Program group name
End Sub
